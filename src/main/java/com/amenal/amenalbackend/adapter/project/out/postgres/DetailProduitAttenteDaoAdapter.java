...
public class DetailProduitAttenteDaoAdapter implements DetailProduitAttenteDao {
	...
	public List<DetailProduitAttente> findAllDetailProduitAttentes() {
		...
		return detailProduitAttenteEntities.stream().map(detailProduitAttenteEntity -> modelMapper.map(detailProduitAttenteEntity, DetailProduitAttente.class))
				.collect(Collectors.toList());
	}

	@Override
	public DetailProduitAttente saveDetailProduitAttente(DetailProduitAttente detailProduitAttente) {		
		DetailProduitAttenteEntity detailProduitAttenteEntity = modelMapper.map(detailProduitAttente, DetailProduitAttenteEntity.class);
	...
	public DetailProduitAttente updateDetailProduitAttente(DetailProduitAttente detailProduitAttente) {
		DetailProduitAttenteEntity existingEntity = detailProduitAttenteRepository.findById(detailProduitAttente.getId()).orElseThrow();
		detailProduitAttente.setErreur(getErreurMessage(detailProduitAttente));

		// Use ModelMapper to map non-null properties from DetailProduitAttente to existingEntity
	...
	private String getErreurMessage(DetailProduitAttente detailProduitAttente) {
		...
		String erreur = (detailProduitAttente.getProduit() == null || detailProduitAttente.getProduit() == ""
				...
				+ (detailProduitAttente.getActivite() == null || detailProduitAttente.getActivite() == ""
						? "| Activite Vide "
						: "")
				+ (detailProduitAttente.getUpb() == null || detailProduitAttente.getUpb() == ""
				? "| Unite Vide "
				: "");
		if (erreur != "")
			return "(1)" + erreur;
		...
		List<TacheEntity> tachesInOtherAvenantEntities = tacheRepository.getTachesInOtherAvenants(detailProduitAttente.getAvenant().getId());
		...
		List<TacheEntity> tachesInSameAvenantEntities = tacheRepository.getTachesByAvenantId(detailProduitAttente.getAvenant().getId());
		...
		List<DetailProduitAttenteEntity> otherDetailEntities = detailProduitAttenteRepository.getOtherDetailsById(detailProduitAttente.getId(), detailProduitAttente.getAvenant().getId());
		List<DetailProduitAttente> otherDetailsInAttente = otherDetailEntities.stream().map(detailEntity -> modelMapper.map(detailEntity, DetailProduitAttente.class))
				.collect(Collectors.toList());
		...
		erreur = (detailProduitAttente.getProduit() == null || detailProduitAttente.getProduit() == ""
				? "| Produit Vide "
				: "")
				+ (detailProduitAttente.getNbr() == null || detailProduitAttente.getNbr() == 0
						? "| Nbr null "
						: "");
		if (erreur != "")
			return "(7)" + erreur;
		...
		List<DetailProduitEntity> otherDetailProduitEntities = detailProduitRepository.getDetailProduitsByAvenantId(detailProduitAttente.getAvenant().getId());
		List<DetailProduit> otherDetailProduits = otherDetailProduitEntities.stream().map(detailEntity -> modelMapper.map(detailEntity, DetailProduit.class))
		.collect(Collectors.toList());
		...
		for(DetailProduit detailProduit: otherDetailProduits) {
			if (detailProduit.getTache().getTitreActivite().equalsIgnoreCase(detailProduitAttente.getActivite()) 
					&& detailProduit.getTache().getLot().getDesignation().equalsIgnoreCase( detailProduitAttente.getLot())
					&& detailProduit.getTache().getProduit().getDesignation().equalsIgnoreCase(detailProduitAttente.getProduit())
	...
	public List<DetailProduitAttente> getDetailProduitAttentesByAvenantId(Integer id) {
		List<DetailProduitAttenteEntity> detailProduitAttenteEntities = detailProduitAttenteRepository.getDetailProduitAttentesByAvenantId(id);
